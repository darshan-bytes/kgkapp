...
class WatchlistDetailsBloc extends Bloc<WatchlistDetailsEvent, WatchlistDetailsState> {
  ...
  Future<void> _onWatchlistDetailsInitialEvent(WatchlistDetailsInitialEvent event, Emitter<WatchlistDetailsState> emit) async {
    ...
    if (watchlistId.isNotEmpty) {
      Either<ErrorResponse, WatchlistData>? response = await AppRepository(
        event.context,
      ).getWatchListById(watchlistId, isInBackground: event.isInBackground);
      response?.fold(
        (error) {
          event.context.pop();
  ...
  List<ProductDetailsModel> _generateProductList() {
    return List.generate(watchlistDetailsModel.products?.length ?? 0, (index) {
      ...
      switch (product.displayCommodity) {
        case Commodity.jewellery:
          return ProductDetailsModel(
            ...
            imageUrl:
                product.jewelleryData?.multipleFinishedViewImage.isNotNullNorEmpty == true
            ...
            discountPercentageString:
                (product.jewelleryData?.discountPercentage ?? 0) > 0
  ...
  Future<void> _onWatchlistDetailsEditProductEvent(WatchlistDetailsEditProductEvent event, Emitter<WatchlistDetailsState> emit) async {
    if (event.actionType == WatchlistActionType.edit) {
      BlocProvider.of<AddToWatchlistBloc>(
        event.context,
  ...
  Future<void> _onWatchlistDetailsDeleteEvent(WatchlistDetailsDeleteEvent event, Emitter<WatchlistDetailsState> emit) async {
    ...
    response?.fold(
      ...
      (data) {
        ...
        event.context.pop();
        event.screenContext.pop(arguments: {RoutesData.isWatchlistDeleted: true, RoutesData.watchlistData: watchlistDetailsModel});
      },
    );
  }
  ...
  void handleBack(BuildContext context, {required bool needToPop}) {
    if (needToPop) {
      context.pop(arguments: {RoutesData.isWatchlistUpdated: isWatchlistUpdated, RoutesData.watchlistData: watchlistDetailsModel});
    }
  }
